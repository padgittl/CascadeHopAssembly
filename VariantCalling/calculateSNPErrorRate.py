...
# create an array of zeros that is the length of a single contig
def createCountArray(contigLen):
    countArray = [0]*contigLen
    return(countArray)

# update the count in the array based on the presence of an exon
...
# loop through all exons for each contig and update count array
...
# read vcf file from variant calling pipeline
def readVCF(vcfFile):
    vcfDict = {}
...
# verify that variant occurs within exon
...
# calculate percent error - total number of variants divided by total exon length 
def calculateErrorInFeature(variantTotalCountDict,totalFeatureLenDict,lengthsDict):
    for contigID in variantTotalCountDict:
        contigLen = lengthsDict[contigID]
        outName = contigID + '.snpError.txt'
        OUT = open(outName,'w')
        variantCount = variantTotalCountDict[contigID]
