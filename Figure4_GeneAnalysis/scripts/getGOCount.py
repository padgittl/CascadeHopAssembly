    ...
    with open(geneFile,'r') as GF:
        for line in GF:
            ...
            if goID not in geneDict:
                geneDict[goID] = []
            geneDict[goID].append((primaryGeneID,hapGeneID,dNdS,uniprotID))
...
def combine(goCountDict,goInfoDict,geneDict):
    ...
    for goID in goCountDict:
        ...
        if goID in goInfoDict and goID in geneDict:
            ...
            for primaryGeneID,hapGeneID,dNdS,uniprotID in geneDict[goID]:
                print("%s\t%s\t%s\t%s\t%s\t%s\t%s" % (uniprotID,goID,goDesc,goCount,primaryGeneID,hapGeneID,dNdS))
                #print("%s;%s;%s;%s;%s;%s;%s" % (uniprotID,goID,goDesc,goCount,primaryGeneID,hapGeneID,dNdS))
